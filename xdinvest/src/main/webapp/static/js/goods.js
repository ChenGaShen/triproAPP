...
//			showRefresh : true, // 是否显示刷新按钮
			...
			columns : [ {
	            ...
	            formatter:function(value,row,index){
				  ...
				  field: 'sales', // 返回json数据中的name
	              title: '销量(件/包)', // 表格表头显示文字
	              align: 'center', // 左右居中
	              valign: 'middle', // 上下居中
			},{
...
$("#addButton").click(function () {
	  if ($("#form_data").data('bootstrapValidator').isValid()) {//获取验证结果，如果成功，执行下面代码 
    	   ...
    	   formData.append('name', $("input[name='add_commodityName']").val());
...
$('#btn_delete').click(function(){
    ...
    if(rowData.length<=0){
    	...
    	$("#delete_name").html(rowData[0].name);
...
$('#btn_edit').click(function(){
	  var rowData= $("#t_table").bootstrapTable('getSelections'); 
	    if(rowData.length<=0){
	    	...
	    	$("#update_commodityName").val(rowData[0].name);
...
$("#updateButton").click(function () {
	  ...
	  if (($("#update_form_data").data('bootstrapValidator')).isValid()) {//获取验证结果，如果成功，执行下面代码 
  	   var formData1=new FormData($('#update_form_data'));//使用FormData提交表单并上传文件  var formData=new FormData($('#form_data')[0]);
  	     formData1.append('id', $("input[name='update_commodityId']").val());
  	   	 formData1.append('name', $("input[name='update_commodityName']").val());
